...
const Step12 = ({ formData = {}, handleInputChange, errors = {}, calculateTotalPayment, motorcycles = [] }) => {
  ...
  // Use ROOT formData fields
  const selectedBike = motorcycles.find(bike => bike.name === (formData.motorcycle?.selectedMotorcycle || ''));
  const bikeHire = selectedBike && formData.motorcycle?.hireOption === 'Hire a Motorcycle' ? selectedBike.price * 6 : 0;
          ...
          <div className="bg-gray-100 p-4 rounded-lg mb-4">
            ...
            <div className="mb-4">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  <p className="font-medium text-gray-900">{formData.motorcycle?.selectedMotorcycle || 'N/A'}</p>
                  <p className="text-sm text-gray-600">6 days hire</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-200 p-3 rounded">
              ...
              <div className="text-sm text-gray-600 mt-1">
                Daily Rate: ${selectedBike ? selectedBike.price : 0}/day | 6 days | Collect: 3rd May 2026, Return: 8th May 2026
              </div>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              <input
                type="radio"
                name="payment.paymentOption"  // ← Full dot notation!
                value="Full Payment"
                checked={formData.payment?.paymentOption === 'Full Payment'}
                onChange={handleInputChange}  // ← Just this handler
                className="mr-2 focus:ring-green-500"
              />
              <span>Full Payment (SAVE $50)</span>
            </label>
            <label className="flex items-center">
              <input
                type="radio"
                name="payment.paymentOption"  // ← Same name for radio group
                value="Deposit"
                checked={formData.payment?.paymentOption === 'Deposit'}
