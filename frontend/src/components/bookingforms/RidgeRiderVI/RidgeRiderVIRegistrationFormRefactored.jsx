...
// 2026 changes start here with prefill
...
const RidgeRiderVIRegistrationFormRefactored = () => {

  const { formRef } = useUserAutoFill(USER_FIELDS);
  console.log('RidgeRiderVIRegistrationFormRefactored: Component loaded');
  ...
  // Fetch profile + motorcycles
  useEffect(() => {
    const fetchData = async () => {
      const token = localStorage.getItem('token');

      // 1. Fetch motorcycles (always, even without login)
      try {
        console.log('RidgeRiderVIRegistrationFormRefactored: Fetching bikes');
        const bikeData = await fetchMotorcycles();
        setBikes(bikeData);
        console.log('RidgeRiderVIRegistrationFormRefactored: Bikes fetched', bikeData.length);
      } catch (err) {
        console.error('RidgeRiderVIRegistrationFormRefactored: Fetch bikes error', err.message);
        setApiError('Failed to load motorcycles. Please try again.');
      } finally {
        setBikesLoading(false);
      }

      // 2. Fetch user profile for prefill (only if logged in)
      if (token) {
        try {
          console.log('RidgeRiderVIRegistrationFormRefactored: Fetching user profile for prefill');
          ...
          setFormData(prev => ({
            ...
            country: profile.country || prev.country || 'Australia',

            // Phone related
            phonePlatform: profile.phonePlatform || prev.phonePlatform || '',
            phoneModel: profile.phoneModel || prev.phoneModel || '',
            hasGPS: profile.hasGPS || prev.hasGPS || '',
            hasFacebook: profile.hasFacebook || prev.hasFacebook || '',

            // Emergency contact 1
            emergency1FirstName: profile.emergencyFirstName || prev.emergency1FirstName || '',
            emergency1LastName: profile.emergencyLastName || prev.emergency1LastName || '',
            emergency1Email: profile.emergencyEmail || prev.emergency1Email || '',
            emergency1Mobile: profile.emergencyMobile || prev.emergency1Mobile || '',
            emergency1Landline: profile.emergencyLandline || prev.emergency1Landline || '',
            emergency1Relationship: profile.emergencyRelation || prev.emergency1Relationship || '',

            // Medical - nested in schema
            hasMedicalCondition: profile.medicalInfo?.hasMedicalCondition || prev.hasMedicalCondition || '',
            ...
            hasMedicationAllergies: profile.medicalInfo?.hasMedicationAllergies || prev.hasMedicationAllergies || '',
            medicationAllergies: profile.medicalInfo?.medicationAllergies || prev.medicationAllergies || '',
            hasFoodAllergies: profile.medicalInfo?.hasFoodAllergies || prev.hasFoodAllergies || '',
            ...
            hasHealthFund: profile.medicalInfo?.hasHealthFund || prev.hasHealthFund || '',
            healthFundName: profile.medicalInfo?.healthFundName || prev.healthFundName || '',
            healthFundNumber: profile.medicalInfo?.healthFundNumber || prev.healthFundNumber || '',
            hasAmbulanceCover: profile.medicalInfo?.hasAmbulanceCover || prev.hasAmbulanceCover || '',
            ...
            // Licence (some fields)
            licenceNumber: profile.licenceNumber || prev.licenceNumber || '',
            licenceState: profile.licenceState || prev.licenceState || '',
            // licenceExpiryDate - assuming ISO string from backend
            licenceExpiryDate: profile.licenceExpiry 
              ? new Date(profile.licenceExpiry) 
              : prev.licenceExpiryDate,
            // licenceFile cannot be pre-filled (file input security)
          }));

          console.log('RidgeRiderVIRegistrationFormRefactored: Profile prefilled');
        } catch (err) {
          console.error('RidgeRiderVIRegistrationFormRefactored: Profile fetch failed', err);
        } finally {
...
const handleInputChange = (e) => {
  ...
  setFormData((prev) => {
    // ── 1. Special case: array checkboxes (hydration, etc.) ───────────────
    if (type === 'checkbox' && ['hydration', 'electronicEquipment', 'upperProtective', 'lowerProtective', 'mechanicalRelated'].includes(name.split('.').pop())) {
    ...
    // ── 2. Normal checkbox (agreeToTerms, etc.) ────────────────────────────
    if (type === 'checkbox') {
      return { ...prev, [name]: checked };
    }

    // ── 3. File input (licenceFile) ────────────────────────────────────────
    if (type === 'file') {
      return { ...prev, [name]: files[0] || null };
    }

    // ── 4. Dot notation support (motorcycle.hireOption, motorcycle.ownBike.make, etc.)
    ...
    if (path.length > 1) {
      ...
      // Traverse all levels except the last one
      for (let i = 0; i < path.length - 1; i++) {
        const key = path[i];
        current[key] = { ...(current[key] || {}) }; // shallow copy to avoid mutation
        current = current[key];
      }

      // Set the final value (radio → value, text/select → value)
    ...
    // ── 5. Top-level field (fallback) ──────────────────────────────────────
    return { ...prev, [name]: value };
  });

  // Clear error (use the last part of the name for error key)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create payload WITHOUT licenceDetails (sent separately)
      const payload = {
        personalDetails: {
      ...
      // FormData for multipart (licence file + json fields)
      ...
      // Add licence text fields separately
      ...
      // Add all other JSON payload parts
      ...
      const response = await axios.post('/RidgeRiderVIRegistrations/create', formDataToSend, {
        headers: {
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-6 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-4 px-2">
              Registration Form RidgeRiderVI 2026
            </h1>
...
export default RidgeRiderVIRegistrationFormRefactored;
