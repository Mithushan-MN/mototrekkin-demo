...
// code with prefill developig 2026 start here
...
const MDPPhase3Registration = () => {
  ...
  // Safe plain object ref for autofill data – never attach to DOM
  ...
  const [formData, setFormData] = useState({
    ...
    trainingDate: 'OCTOBER 30th + 31th Phase II Hunter Valley NSW 2322',
    ...
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        setFormData(prev => ({
          ...
          // Personal details – direct mapping from schema
          ...
          // Emergency contact – your schema uses flat fields (not nested object)
          emergency1: {
          ...
          // emergency2 not present in schema – keeping empty or duplicate if needed
          emergency2: prev.emergency2,

          shirtSize: prev.shirtSize, // not in schema

          // Medical information – nested under medicalInfo
          ...
          // licenseValid – not stored in profile schema, controlled by form
          licenseValid: prev.licenseValid,

          // Other registration-specific fields remain empty / default
          // (bike details, tyres, payment, terms, etc. are not in userProfile)
        }));

        // Optional: mirror to safe ref (only if some step components still expect it)
  ...
  const handleInputChange = (field, value) => {
    ...
    // Safe mirror – only to plain ref (no DOM writes)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("Sending birthday value:", formData.birthday);
console.log("Type of birthday:", typeof formData.birthday);
console.log("Full payload personalDetails.birthday:", payload.personalDetails.birthday);

      const response = await axios.post('/MDPPhase3Registrations/create', payload, {
        headers: {
        ...
        <div className="flex justify-between">
          ...
          {formData.completedPhase1 === 'Yes' && (
            <button
              ...
              className={`px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors ${fetchingProfile ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {fetchingProfile ? 'Loading...' : 'Next Step'}
            </button>
          )}
        </div>
      </div>
  ...
  const renderCurrentStep = () => {
    ...
    return (
      <CurrentStepComponent
        ...
        navigate={navigate}
        formRef={formRef} // kept for compatibility – but we no longer write to it
      />
    );
  };
...
export default MDPPhase3Registration;