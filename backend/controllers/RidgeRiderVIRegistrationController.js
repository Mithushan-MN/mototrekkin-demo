// controllers/RidgeRiderVIRegistrationController.js
import { Stripe } from 'stripe';
import RidgeRiderVIRegistration from '../models/RidgeRiderVIRegistration.js';
...
// CREATE
// ─────────────────────────────────────────────────────────────────────────────
export const createRidgeRiderVIRegistration = async (req, res) => {
  try {
    const userId = req.user.id;
    console.log('createRidgeRiderVIRegistration: Creating registration', {
      userId,
      body: req.body,
      file: req.file ? {
  ...
  console.log('createRidgeRiderVIRegistration: Validation FAILED', validationErrors);
...
console.log('createRidgeRiderVIRegistration: ALL VALIDATION PASSED');
...
const licenceFileName = req.file.originalname || 'licence-file';
    ...
    const registration = new RidgeRiderVIRegistration({
      userId,
    ...
    console.log('createRidgeRiderVIRegistration: Registration SAVED', { registrationId: registration._id });
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'aud',
          product_data: {
            name: 'RidgeRiderVIRegistration 2026 Registration',
    ...
    console.log('createRidgeRiderVIRegistration: Stripe session CREATED', { sessionId: session.id });
    ...
    try {
      await transporter.sendMail({
        ...
        subject: 'RidgeRiderVIRegistration 2026 Registration Confirmation',
        html: `
          <h2>Registration Confirmation</h2>
          <p>Dear ${parsedData.personalDetails?.firstName},</p>
          <p>Your registration for RidgeRiderVI 2026 has been received.</p>
          <p>Total Payment: $${totalPayment.toFixed(2)}</p>
          <p><a href="${session.url}">Complete Payment Here</a></p>
          <p>Registration ID: ${registration._id}</p>
        `,
      });
      console.log('createRidgeRiderVIRegistration: User email SENT');
      emailStatus.userEmailSent = true;
    } catch (err) {
      console.error('createRidgeRiderVIRegistration: User email ERROR', err.message);
    ...
    try {
      await transporter.sendMail({
        ...
        subject: 'New RidgeRiderVI Registration',
        html: `
      ...
      console.log('createRidgeRiderVIRegistration: Admin email SENT');
      emailStatus.adminEmailSent = true;
    } catch (err) {
      console.error('createRidgeRiderVIRegistration: Admin email ERROR', err.message);
    ...
    console.error('createRidgeRiderVIRegistration: ERROR', {
...
export const getAllRegistrations = async (req, res) => {
  try {
    console.log('getAllRegistrations: Fetching all RidgeRiderVI registrations');
    const registrations = await RidgeRiderVIRegistration.find()
...
export const getUserRegistrations = async (req, res) => {
  try {
    ...
    const registrations = await RidgeRiderVIRegistration.find({ userId })
      .sort({ createdAt: -1 });
...
// UPDATE (ADMIN)
// ─────────────────────────────────────────────────────────────────────────────
export const updateRidgeRiderVIRegistration = async (req, res) => {
  try {
    const { id } = req.params;
    const existing = await RidgeRiderVIRegistration.findById(id);
    ...
    const updated = await RidgeRiderVIRegistration.findByIdAndUpdate(
      id,
      updateObj,
    ...
    if (updated.payment.paymentStatus === 'Paid' && existing.payment.paymentStatus !== 'Paid') {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{
          price_data: {
            currency: 'aud',
            product_data: { name: 'RidgeRiderVI Payment (Admin)' },
            unit_amount: Math.round(totalPayment * 100),
          },
          quantity: 1,
        }],
        mode: 'payment',
        success_url: `${origin}/admin/RidgeRiderVI-registrations`,
        cancel_url: `${origin}/admin/RidgeRiderVI-registrations`,
    ...
    try {
      await transporter.sendMail({
        ...
        subject: 'RidgeRiderVI Registration Updated',
    ...
    console.error('updateRidgeRiderVIRegistration: ERROR', error);
...
// DELETE
// ─────────────────────────────────────────────────────────────────────────────
export const deleteRidgeRiderVIRegistration = async (req, res) => {
  try {
    const { id } = req.params;
    const reg = await RidgeRiderVIRegistration.findById(id);
    ...
    await RidgeRiderVIRegistration.findByIdAndDelete(id);
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'aud',
          product_data: { name: 'RidgeRiderVI Payment (Resend)' },
