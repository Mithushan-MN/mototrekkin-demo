import MDPPhase1Registration from '../models/MDPPhase1Registration.js';
import MDPPhase1Bike from '../models/MDPPhase1Bike.js'; // Added import
...
export const createMDPPhase1Registration = async (req, res) => {
  try {
    const userId = req.user.id;
    console.log('createMDPPhase1Registration: Starting', { userId });
...
if (!payload.trainingDate?.trim()) errors.push('Training date is required');
    ...
    if (payload.bikeDetails?.hireBike) {
      ...
      const updatedBike = await MDPPhase1Bike.findOneAndUpdate(
        { name: bikeName, isActive: true },
        { $inc: { remaining: -1 } },
        { new: true }
      );
      if (!updatedBike || updatedBike.remaining < 0) {
        throw new Error(`MDP Phase 1 Bike ${bikeName} is no longer available`);
      }
      console.log(`MDP Phase 1 Bike ${bikeName} remaining updated to ${updatedBike.remaining}`);
    }

    // ──────── SAVE TO DB ────────
    const registration = new MDPPhase1Registration({
      userId,
      ...payload,
      payment: {
        ...payload.payment,
    ...
    const session = await stripeInstance.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'aud',
          product_data: { name: 'MDP Phase I Registration' },
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        subject: 'MDP Phase I – Registration Received',
        html: `
    ...
    try {
      await transporter.sendMail({
        ...
        subject: 'New MDP Phase I Registration',
        html: `
    ...
    console.error('MDPPhase1Registration ERROR:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// GET: Fetch all MDP Phase 2 registrations for the logged-in user
export const getUserMDPPhase1Registrations = async (req, res) => {
  try {
    const userId = req.user.id;
    console.log('getUserMDPPhase1Registrations: Fetching for user', userId);

    const registrations = await MDPPhase1Registration.find({ userId })
    ...
    console.error('getUserMDPPhase1Registrations ERROR:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch registrations' });
  }
};

export const getAllMDPPhase1Registrations = async (req, res) => {
  try {
    const regs = await MDPPhase1Registration.find()
      .sort({ createdAt: -1 })
      .lean();
    res.json(regs);
  } catch (err) {
...
export const deleteMDPPhase1Registration = async (req, res) => {
  try {
    await MDPPhase1Registration.findByIdAndDelete(req.params.id);
...
export const updateMDPPhase1Registration = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const updated = await MDPPhase1Registration.findByIdAndUpdate(
      id,
      { $set: updates },
