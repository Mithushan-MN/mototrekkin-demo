import MDPPhase1Bike from '../models/MDPPhase1Bike.js';

// GET: Fetch available bikes
export const getBikes = async (req, res) => {
  try {
    const bikes = await MDPPhase1Bike.find({ isActive: true }).sort({ name: 1 });
...
export const updateBike = async (req, res) => {
  try {
    ...
    const bike = await MDPPhase1Bike.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
...
export const deleteBike = async (req, res) => {
  try {
    const { id } = req.params;
    const bike = await MDPPhase1Bike.findByIdAndUpdate(
      id,
      { isActive: false },
      { new: true }
    );
