import MDPPhase3Registration from '../models/MDPPhase3Registration.js';
import MDPPhase3Bike from '../models/MDPPhase3Bike.js'; // Added import
...
export const createMDPPhase3Registration = async (req, res) => {
  try {
    const userId = req.user.id;
    console.log('createMDPPhase3Registration: Starting', { userId });
...
if (!payload.trainingDate?.trim()) errors.push('Training date is required');
    ...
    if (payload.bikeDetails?.hireBike) {
      ...
      const updatedBike = await MDPPhase3Bike.findOneAndUpdate(
        { name: bikeName, isActive: true },
        { $inc: { remaining: -1 } },
        { new: true }
      );
      if (!updatedBike || updatedBike.remaining < 0) {
        throw new Error(`MDP Phase 3 Bike ${bikeName} is no longer available`);
      }
      console.log(`MDP Phase 3 Bike ${bikeName} remaining updated to ${updatedBike.remaining}`);
    }

    // ──────── SAVE TO DB ────────
    const registration = new MDPPhase3Registration({
      userId,
      ...payload,
      payment: {
        ...payload.payment,
    ...
    console.error('MDPPhase3Registration ERROR:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// GET: Fetch all MDP Phase 2 registrations for the logged-in user
export const getUserMDPPhase3Registrations = async (req, res) => {
  try {
    const userId = req.user.id;
    console.log('getUserMDPPhase3Registrations: Fetching for user', userId);

    const registrations = await MDPPhase3Registration.find({ userId })
      .select('-__v')                    // Remove version key
      .sort({ createdAt: -1 })           // Newest first
      .lean();                           // Faster, plain JS objects

    console.log(`Found ${registrations.length} MDP Phase 3 registration(s)`);
    res.status(200).json(registrations);
  } catch (error) {
    console.error('getUserMDPPhase3Registrations ERROR:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch registrations' });
  }
};

export const getAllMDPPhase3Registrations = async (req, res) => {
  try {
    const regs = await MDPPhase3Registration.find()
      .sort({ createdAt: -1 })
      .lean();
    res.json(regs);
  } catch (err) {
...
export const deleteMDPPhase3Registration = async (req, res) => {
  try {
    await MDPPhase3Registration.findByIdAndDelete(req.params.id);
...
export const updateMDPPhase3Registration = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const updated = await MDPPhase3Registration.findByIdAndUpdate(
      id,
      { $set: updates },
